...
void build_3D_map::cloud_cb (const sensor_msgs::PointCloud2ConstPtr& input) 
{
  ...
  for(i=uu_uav;i<globalMapWidth;i++)
  {
    ...
    if(num_obstacle >10) //若是墙，则该行不少于20
    {
    ...
    for(size_t vv_temp=0; vv_temp<globalMapLongth-20; vv_temp++) //vv_temp设置为了防止数组越界
    {
      ...
      if(obstacle_front_map.maparry[uu_wall_min+vv_temp*globalMapWidth+ww_temp*globalMapLongth*globalMapLongth] == 1) //找到当前的障碍
      {
        ...
        obstacle_front_map.maparry[uu_wall_min+(vv_temp+20)*globalMapWidth+ww_temp*globalMapLongth*globalMapLongth] == 1) 
        {
            ...
            else if(obstacle_front_map.maparry[uu_wall_min+(vv_temp+12)*globalMapWidth+ww_temp*globalMapLongth*globalMapLongth] == 0 &&
  ...
  else
  {
    ...
    else if(flight_command_msg.front_wall_reach)
    {
      if(flight_command_msg.window_is_left) 
        ...
        if(obstacle_front_map.maparry[uu_wall_min+vv_uav*globalMapWidth+(ww_uav+1)*globalMapLongth*globalMapLongth]==1 &&
          obstacle_front_map.maparry[uu_wall_min+(vv_uav+1)*globalMapWidth+(ww_uav+1)*globalMapLongth*globalMapLongth]==0 &&
          obstacle_front_map.maparry[uu_wall_min+(vv_uav-2)*globalMapWidth+(ww_uav+1)*globalMapLongth*globalMapLongth]==1 && 
          obstacle_front_map.maparry[uu_wall_min+(vv_uav+2)*globalMapWidth+(ww_uav+1)*globalMapLongth*globalMapLongth]==0 &&
          obstacle_front_map.maparry[uu_wall_min+(vv_uav-1)*globalMapWidth+(ww_uav-1)*globalMapLongth*globalMapLongth]==1 &&
          obstacle_front_map.maparry[uu_wall_min+(vv_uav+1)*globalMapWidth+(ww_uav-1)*globalMapLongth*globalMapLongth]==0 &&
          obstacle_front_map.maparry[uu_wall_min+(vv_uav-2)*globalMapWidth+(ww_uav-1)*globalMapLongth*globalMapLongth] ==1&& 
          obstacle_front_map.maparry[uu_wall_min+(vv_uav+2)*globalMapWidth+(ww_uav-1)*globalMapLongth*globalMapLongth]==0 
          )
          action_command_msgs.data=UP;
        else if(obstacle_front_map.maparry[uu_wall_min+(vv_uav-1)*globalMapWidth+(ww_uav+1)*globalMapLongth*globalMapLongth]==0 &&
          obstacle_front_map.maparry[uu_wall_min+(vv_uav+1)*globalMapWidth+(ww_uav+1)*globalMapLongth*globalMapLongth] ==1&&
          obstacle_front_map.maparry[uu_wall_min+(vv_uav-2)*globalMapWidth+(ww_uav+1)*globalMapLongth*globalMapLongth] ==0&& 
          obstacle_front_map.maparry[uu_wall_min+(vv_uav+2)*globalMapWidth+(ww_uav+1)*globalMapLongth*globalMapLongth] ==1&&
          obstacle_front_map.maparry[uu_wall_min+(vv_uav-1)*globalMapWidth+(ww_uav-1)*globalMapLongth*globalMapLongth] ==0 &&
