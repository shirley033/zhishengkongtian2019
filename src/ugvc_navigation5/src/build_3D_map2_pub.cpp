...
void build_3D_map::cloud_cb (const sensor_msgs::PointCloud2ConstPtr& input) 
{
  ...
  //先从和无人机前方的uu开始积分
  for(i=uu_uav;i<uu_uav+15;i++)//只扫描前方三米
  {
    for(size_t j=0;j<globalMapLongth;j++)
    { 
      //从y方向来看，正负不超过2即可 
  ...
  for(size_t ww_temp=0; ww_temp<globalMapHeight; ww_temp++)
  {
    ...
    for(size_t vv_temp=0; vv_temp<globalMapLongth-10; vv_temp++) //vv_temp设置为了防止数组越界
    {
      ...
      if(obstacle_front_map.maparry[uu_wall_start+vv_temp*globalMapWidth+ww_temp*globalMapLongth*globalMapLongth] == 1) //找到当前的障碍
      {
        ...
        (obstacle_front_map.maparry[uu_wall_start+(vv_temp+6)*globalMapWidth+ww_temp*globalMapLongth*globalMapLongth] == 1 ||
  ...
  if(obstacle_front_map.maparry[uu_wall_min+vv_uav*globalMapWidth+ww_uav*globalMapLongth*globalMapLongth]==0 && 
    ...
    obstacle_front_map.maparry[uu_wall_min+(vv_uav+2)*globalMapWidth+ww_uav*globalMapLongth*globalMapLongth]==0 &&
    obstacle_front_map.maparry[uu_wall_min+vv_uav*globalMapWidth+(ww_uav+1)*globalMapLongth*globalMapLongth]==0 && 
